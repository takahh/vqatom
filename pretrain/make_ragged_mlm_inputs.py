...
path0 = "/Users/taka/Documents/pretrain_1M.tar.gz"
...
with tarfile.open(path0, "r:gz") as tar:
    names = tar.getnames()
pat = re.compile(r"discret_50k/attr_(\d+)\.npy$")
...
with tarfile.open(path0, "r:gz") as tar:
    for b in batches:
        attr_name = f"discret_50k/attr_{b}.npy"
        smiles_name = f"discret_50k/smiles_{b}.txt"

        attr = np.load(io.BytesIO(read_bytes_from_tar(tar, attr_name)), allow_pickle=True)

        # ---- accept both (n_mols, MAX_ATOMS, ATTR_DIM) and (n_mols*MAX_ATOMS, ATTR_DIM)
        if attr.ndim == 3:
            n_mols, max_atoms, attr_dim = attr.shape
            if max_atoms != MAX_ATOMS or attr_dim != ATTR_DIM:
                raise RuntimeError(f"attr shape mismatch at batch {b}: {attr.shape} expected (*,{MAX_ATOMS},{ATTR_DIM})")
            attr3 = attr  # (n_mols, MAX_ATOMS, ATTR_DIM)

        elif attr.ndim == 2:
            n_rows, attr_dim = attr.shape
            if attr_dim != ATTR_DIM:
                raise RuntimeError(f"attr shape mismatch at batch {b}: {attr.shape} expected (*,{ATTR_DIM})")
            if n_rows % MAX_ATOMS != 0:
                raise RuntimeError(f"attr rows not divisible by MAX_ATOMS at batch {b}: rows={n_rows} MAX_ATOMS={MAX_ATOMS}")
            n_mols = n_rows // MAX_ATOMS
            attr3 = attr.reshape(n_mols, MAX_ATOMS, ATTR_DIM)

        else:
            raise RuntimeError(f"attr ndim unexpected at batch {b}: {attr.shape}")
        ...
        # row_is_real: (n_mols, MAX_ATOMS)
        row_is_real = (np.abs(attr3).sum(axis=2) != 0)
