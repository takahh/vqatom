...
out_dir = "/Users/taka/Documents/pretrain_ragged_continuous"
...
with tarfile.open(path0, "r:gz") as tar:
    for b in batches:
        ...
        if atom_feats_flat.shape[0] != tokens_flat.shape[0]:
            raise RuntimeError(
                f"atom_feats_flat rows != tokens_flat len: "
                f"{atom_feats_flat.shape[0]} vs {tokens_flat.shape[0]}"
            )
