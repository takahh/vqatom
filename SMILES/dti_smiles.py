...
class DTIDataset(Dataset):
    def __init__(
        ...
        smiles_tokenizer: Optional[SmilesCharTokenizer] = None,
        smiles_max_len: Optional[int] = None,
        y_reg_mean: Optional[float] = None,
        y_reg_std: Optional[float] = None,
    ):
        if smiles_tokenizer is None:
            raise ValueError("smiles_tokenizer is required")
    ...
    def __getitem__(self, idx):
        row = self.rows[idx]
        lig_ids = self.smiles_tokenizer.encode(
            row["smiles"],
            add_cls=True,
...
def collate_fn(samples, esm_tokenizer, lig_pad):
    ...
    y_reg = (
        torch.tensor([float(s["y_reg"]) for s in samples], dtype=torch.float32)
        if has_y_reg else None
    )
...
# Encoders
# =========================================================
class PretrainedSmilesEncoder(nn.Module):
    def __init__(
        self,
        ckpt_path: str,
        vocab_json: str,
        ...
        if not (isinstance(ckpt, dict) and "model" in ckpt and "config" in ckpt):
            raise RuntimeError("SMILES checkpoint must contain keys: model, config")

        self.state = ckpt["model"]
        self.conf = ckpt["config"]
        self.tokenizer = SmilesCharTokenizer.from_json(vocab_json)
        ...
        max_len = int(self.conf.get("max_len", 512))
...
def eval_metrics(y_pred: np.ndarray, y_bin: np.ndarray) -> Dict[str, float]:
    ...
    if y_pred.size == 0:
        return {
            "auroc": 0.0, "ap": 0.0, "f1": 0.0, "thr": CLS_THR,
    ...
    if len(np.unique(y01)) <= 1:
        return {
            "auroc": 0.0, "ap": 0.0, "f1": 0.0, "thr": float(CLS_THR),
            "pred_mean": float(score.mean()), "pred_std": float(score.std()),
            "pred_min": float(score.min()), "pred_max": float(score.max()),
...
class DualStreamBlock(nn.Module):
    ...
    def forward(self, p_h, l_h, p_pad=None, l_pad=None, return_maps=False):
        ...
        l_ctx, aux_lp = self.lig_from_prot(
            q_in=l_q, k_in=p_kv, v_in=p_kv, kv_pad_mask=p_pad, return_maps=True
        )
        p_ctx, aux_pl = self.prot_from_lig(
            q_in=p_q, k_in=l_kv, v_in=l_kv, kv_pad_mask=l_pad, return_maps=True
        )
...
class DualStreamDTIClassifier(nn.Module):
    def __init__(
        ...
        ligand_encoder: PretrainedSmilesEncoder,
        dropout: float,
        n_heads: int = 4,
        n_layers: int = 2,
        ...
        detach_attn_for_value: bool = False,
        ...
        feat_dim = self.d_model * (6 if self.use_cls_in_head else 4)
    ...
    def forward(self, p_input_ids, p_attn_mask, l_ids, return_maps: bool = False):
        ...
        if return_maps:
            p_ctx, l_ctx, inter_aux = self.interaction(
                p_h=p_tok, l_h=l_tok, p_pad=p_pad, l_pad=l_pad, return_maps=True
            )
        else:
            p_ctx, l_ctx = self.interaction(
                p_h=p_tok, l_h=l_tok, p_pad=p_pad, l_pad=l_pad, return_maps=False
            )
            inter_aux = {}
        ...
        feat = (
            torch.cat([p_cls, l_cls, p_mean, p_max, l_mean, l_max], dim=-1)
            if self.use_cls_in_head
            else torch.cat([p_mean, p_max, l_mean, l_max], dim=-1)
        )
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        y_reg = batch.y_reg.to(device, non_blocking=True).float() if batch.y_reg is not None else None

        optimizer.zero_grad(set_to_none=True)

        if use_amp:
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logit, yhat_reg, aux = model(p_ids, p_msk, l_ids, return_maps=need_maps)
                ...
                if attn_entropy_lambda != 0.0:
                    # optional, not implemented in aux by default
                    loss_entropy = torch.tensor(0.0, device=device)
...
def save_dti_checkpoint(path: str, model: nn.Module, args: argparse.Namespace, lig_enc: PretrainedSmilesEncoder, epoch: int, best: dict) -> None:
    torch.save(
        {
            "epoch": epoch,
...
def main():
    ...
    ap.add_argument("--smiles_vocab_json", type=str, required=True)
    ap.add_argument("--smiles_max_len", type=int, default=512)
    ...
    ap.add_argument("--out_dir", type=str, default="./dti_out_smiles")
    ...
    ap.add_argument("--attn_activation", type=str, default="softmax", choices=["softmax", "sigmoid"])
    ...
    lig_enc = PretrainedSmilesEncoder(
        ckpt_path=args.lig_ckpt,
        vocab_json=args.smiles_vocab_json,
        device=device,
    ...
    model = DualStreamDTIClassifier(
        ...
        pair_gate_threshold=args.pair_gate_threshold,
    ...
    if args.use_train_valid_csv:
        ...
        if args.train_size is None:
            fixed_train_ds = DTIDataset(
                ...
                drop_missing_y=True,
                smiles_tokenizer=lig_enc.tokenizer,
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if final_eval_loader is not None:
            ...
            f_r = eval_reg_metrics(yhatr_f, yr_f)
            rec["final_eval_metrics"] = f_m
