...
try:
    # Preferred for VQ-Amino. Provide this in utils.py if you want per-AA K.
    from utils import AMINO_CBDICT as CBDICT
except Exception:
    try:
        from utils import CBDICT  # backward-compatible fallback
    except Exception:
        # Default: 20 canonical AA ids plus 0 for PAD/UNK.
        # Override by defining AMINO_CBDICT in utils.py.
        CBDICT = {str(i): 128 for i in range(1, 21)}
...
def kmeans(
    ...
    group_names=None,             # optional labels for heads (e.g., ["C","N","O",...])
):
    """
    ...
    Returns:
        ...
        used_per_label: dict[str,int] # only if group_names provided, else None
    """
    import math
    import torch
    ...
    if group_names is not None:
        if len(group_names) != H:
            raise ValueError(f"group_names must have length H={H}, got {len(group_names)}")
        used_per_label = {group_names[h]: int(used_per_head[h].item()) for h in range(H)}
...
class ContrastiveLoss(nn.Module):
    def __init__(self, latent_dim, residue_feat_dim, margin=0.5, temperature=0.1, init_sigmoid_base=0.5):
        super().__init__()
        self.margin = nn.Parameter(torch.tensor(margin))
        self.temperature = temperature
        self.sigmoid_base = nn.Parameter(torch.tensor(init_sigmoid_base))
        self.layer_norm_z = nn.LayerNorm(latent_dim)
        self.layer_norm_residue = nn.LayerNorm(latent_dim)
        args = get_args()
...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        self.cb_dict = CBDICT  # {amino_acid_key(str or int): K_e}
    ...
    def init_embed_(self, data, logger, epoch, mask_dict=None, use_cosine_sim: bool = False):
        """
        Initialize per-amino-acid codebooks (absolute K from self.cb_dict).
        ...
        for raw_key in sorted(mask_dict.keys(), key=lambda x: str(x)):
            ...
            if need_init:
                ...
                logger.info(f"[init_embed_] AA={skey} N={N_i} K_req={K_req} K_run={K_run} K_used={nz}/{K_req}")
    ...
    def forward(
        ...
        """
        Per-amino-acid vector quantization codebook forward (Option A: unified L2-normalized space).
        ...
        # ------------------------------------------------------------------
        # 6) rebuild quantized tensor in original residue order
        # ------------------------------------------------------------------
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        self.lamb_equiv_residue = lamb_equiv_residue
    ...
    def commitment_loss(
        ...
        """
        Per-amino-acid commitment + codebook + repel losses (assignment-consistent).
    ...
    def forward(
        ...
        """
        Forward pass with per-amino-acid quantization + EMA update + commitment loss.
