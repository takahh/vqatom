...
def run_inductive(
    ...
    for epoch in range(1, conf["max_epoch"] + 1):
        ...
        np.savez(f"./sample_adj_{epoch}", sample_list_test[2].cpu()[:200, :200])
        np.savez(f"./sample_bond_num_{epoch}", sample_list_test[3].cpu()[:200])
        np.savez(f"./sample_src_{epoch}", sample_list_test[4].cpu()[:200])
        np.savez(f"./sample_dst_{epoch}", sample_list_test[5].cpu()[:200])
        # np.savez(f"./sample_hop_type_{epoch}", None)
        np.savez(f"./sample_adj_base_{epoch}", sample_list_test[7].cpu()[:200])
