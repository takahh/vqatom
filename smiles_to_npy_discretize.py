...
def mol_to_graph_with_labels(mol, idx: int = 0):
    if mol is None:
        raise ValueError(f"mol is None at idx={idx}")

    # base_mol を defensive copy
    ...
    for i in range(n_heavy):
        try:
            hcount = base_mol.GetAtomWithIdx(i).GetTotalNumHs()
        except Exception:
            hcount = 0
        hcount_list.append(hcount)
    mol = Chem.Mol(base_mol)
    try:
        Chem.SanitizeMol(mol)
    except Exception:
        # ここで完全に落とさず、可能な範囲で進める
        pass
